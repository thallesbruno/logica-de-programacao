...
class ConectarDB:
    ...
    def exportacao_registros(self, rowid):
        with io.open('banco.dump.sql', 'w') as d:
...
class Janela(tk.Frame):
    ...
    def criar_widgets(self):
        ...
        button_adicionar = tk.Button(
            frame1, text='Adicionar', bg='blue', fg='white')
        ...
        button_exportar = tk.Button(
            frame1, text='Exportar', bg='purple', fg='white')
        button_exportar['command'] = self.exportar_registro
        button_exportar.grid(row=0, column=4, rowspan=2, padx=10)

        # Treeview.
        self.treeview = tkk.Treeview(
            frame2, columns=('N° documento', 'Assunto', 'Data'))
        ...
        button_excluir = tk.Button(
            frame3, text='Excluir', bg='red', fg='white')
        # Método que é chamado quando o botão é clicado.
    ...
    def adicionar_registro(self):
        ...
        if validar_data:
            ...
            self.banco.inserir_registro(
                ndocumento=documento, assunto=assunto, data=data)
            ...
            self.treeview.insert('', 'end', text=rowid,
                                 values=(documento, assunto, data))
        else:
            # Caso a data não passe na validação é exibido um alerta.
            messagebox.showerror(
                'Erro', 'Padrão de data incorreto, utilize dd/mm/yyyy')
    ...
    def exportar_registro(self):
        self.banco.exportacao_registros(rowid['text'])
